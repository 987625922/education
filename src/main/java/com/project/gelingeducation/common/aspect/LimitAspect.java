...
import com.project.gelingeducation.common.domain.LimitType;
...
public class LimitAspect {
    ...
    @Pointcut("@annotation(com.project.gelingeducation.common.annotation.Limit)")
    ...
    public Object around(ProceedingJoinPoint point) throws Throwable {
        ...
        Number count = limitRedisTemplate.execute(redisScript, keys, String.valueOf(limitCount),
                        String.valueOf(limitPeriod));
        log.info("IP:{} 第 {} 次访问key为 {}，描述为 [{}] 的接口", ip, count, keys, name);
        if (count != null && count.intValue() <= limitCount) {
            return point.proceed();
        } else {
            throw new AllException(StatusEnum.LIMIT_ACCESS_EXCEPTION);
        }
    }
